...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        rays_o = rays_o + torch.randn(3, device=self.device) * self.opt.known_view_noise_scale*2.5
        rays_d = rays_d + torch.randn(3, device=self.device) * self.opt.known_view_noise_scale*2.5
        ambient_ratio = 1.0
        shading = 'albedo'
        binarize = False
        ...
        loss = F.mse_loss(pred_rgb[0,:,:,:], gt_rgb).mean(-1).mean()
