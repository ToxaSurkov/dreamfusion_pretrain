...
def rand_poses(size, device, radius_range=[1.2, 1.2], theta_range=[90, 90], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, jitter=False, uniform_sphere_rate=0.5):
...
class NeRFDataset:
    def __init__(self, opt, device, type='train', H=256, W=256, size=100):
        ...
        for f in tqdm.tqdm(range(50), desc=f'Loading {type} data'):
            f_path = os.path.join('/stable-dreamfusion/data/car', 'images', str(f)+'.png')    
            #print('i m here')
            image = cv2.imread(f_path, cv2.IMREAD_UNCHANGED)
            #print(f_path)# [H, W, 3] o [H, W, 4]
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            poses, dirs, thetas, phis, radius = rand_poses(B, self.device, phi_range=[-90+((index[0]%50) / 50) * 360, -90+((index[0]%50) / 50) * 360], return_dirs=True, angle_overhead=self.opt.angle_overhead, angle_front=self.opt.angle_front, jitter=self.opt.jitter_pose, uniform_sphere_rate=0)
        ...
        data = {
            ...
            'radius': delta_radius,
            'ind': index,
            'gt_rgb' : self.images[index[0]%50].to('cuda')
        }
